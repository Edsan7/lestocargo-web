...
namespace LestoCargo.Admin
{
    public partial class SchedulePayment : System.Web.UI.Page
    {
        ...
        void RecuperarDados()
        {
            try
            {
                string comando = "SELECT Codigo,Nome,Descricao,Nota_Fiscal AS [Nota Fiscal],Data_Pagamento AS Data FROM Pedido WHERE Status='Agendar Pagamento' ORDER BY Data_Pagamento ASC";
        ...
        protected void Pedido_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tb.Rows.Count == 1)
                {
                    ...
                    NotaFiscal.Text += tb.Rows[0]["Nota_Fiscal"].ToString();
